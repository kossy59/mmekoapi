...
let userdb = require("../Creators/creators")
 ...
 let earning_history = async (creator_portfoliio_Id)=>{

   // let user = await creatordb.findOne({_id : creator_portfoliio_Id}).exec()

    let earning = await earningdb.find({userid : creator_portfoliio_Id}).exec() 
