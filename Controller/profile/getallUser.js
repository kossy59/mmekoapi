...
let deleteAcc = async(req, res)=>{
    ...
    if(list_of_users.length > 0){
        ...
        list_of_users.forEach(value=>{
            ...
            let photo = ""
            let photos = user_photos.find(value1=> String(value._id) === String(value1.useraccountId))
            if(photos.photoLink){
        ...
        return res.status(200).json({"ok":true,'message': 'Fectch Success', users:users})
    }else{
        return res.status(200).json({"ok":true,'message': 'Fectch Success', users:[]})
