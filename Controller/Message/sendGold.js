...
const createModel = async (req, res) => {
  ...
  if (!userid || !modelid) {
    return res.status(400).json({ ok: false, message: "User or Model ID missing!" });
  }

  try {
    const user = await userdb.findById(userid).exec();
    const withdraw = await userdb.findById(modelid).exec(); // assuming modelid is also a user ID for withdrawal
    ...
    if (!withdraw) {
      return res.status(400).json({ ok: false, message: "Withdraw user not found!" });
    }

    const user_balance = parseFloat(user.balance);
    const gold_amount = parseFloat(amount);
    ...
    // Deduct from user
    user.balance = `${user_balance - gold_amount}`;
    await user.save();
    ...
    model_as_user.earnings = (model_as_user.earnings ?? 0) + gold_amount;
    await model_as_user.save();

    // Update withdraw balance
    let withdraw_balance = parseFloat(withdraw.withdrawbalance || 0);
    withdraw.withdrawbalance = `${withdraw_balance + gold_amount}`;
    await withdraw.save();
...
// ✅ FIXED: Now searches by model `_id`, not `userid`
const get_model_userID = async (modelid) => {
  return await modeldb.findById(modelid).exec();
};
