...
//     // Count how many posts this user made today
...
//       const originalExt = mime.extension(req.file.mimetype) || "mp4";
...
//       // Trim video to 3 minutes (180 seconds)
...
//       // Upload trimmed file to Cloudinary
...
const createPost = async (req, res) => {
  ...
  try {
    ...
    if (req.file && posttype === "image") {
      const tempDir = os.tmpdir();
      const originalExt = mime.extension(req.file.mimetype) || "jpg";
      const inputPath = path.join(tempDir, `input-${Date.now()}.${originalExt}`);
      const outputPath = path.join(tempDir, `compressed-${Date.now()}.webp`);
      ...
      await new Promise((resolve, reject) => {
        ffmpeg(inputPath)
          .outputOptions([
            "-q:v 50",        // quality (lower = better compression)
            "-preset photo"   // optimize for still images
          ])
          .toFormat("webp")
          .save(outputPath)
          .on("end", resolve)
      ...
      const result = await uploadSingleFileToCloudinary(compressedFile, `post`);

      fs.unlinkSync(inputPath);
      fs.unlinkSync(outputPath);

      if (!result.file_link || !result.public_id) {
        return res.status(500).json({ ok: false, message: "Image compression failed" });
      }
    ...
    if (req.file && posttype === "video") {
      ...
      await new Promise((resolve, reject) => {
        ffmpeg(inputPath)
          .setStartTime("00:00:00")
          .setDuration(180)         // ⏱️ Trim to 3 minutes
          .videoCodec("libx265")    // 🎥 Compress to H.265/HEVC
          .audioCodec("aac")
          .outputOptions([
      ...
      if (!result.file_link || !result.public_id) {
        return res.status(500).json({ ok: false, message: "Video compression failed" });
      }
