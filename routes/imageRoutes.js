...
const {
  ...
  updateImage,
  downloadImage
} = require('../utiils/appwrite'); // Import functions from appwrite.js

const router = express.Router();
const upload = multer({ dest: 'uploads/' }); // Set up multer to handle file uploads

// POST route to upload an image and save it to appwrite
router.post('/save', upload.single('image'), async (req, res) => {
  try {
    ...
    // Save image to appwrite and get the public_id
...
// GET route to download image from appwrite using public_id
router.get('/download', async (req, res) => {
  try {
    ...
    const url = downloadImage(publicId);  // Get the appwrite URL
    res.json({ url });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
// DELETE route to delete an image from appwrite using public_id
router.delete('/delete', async (req, res) => {
  try {
    ...
    await deleteImage(publicId);  // Delete the image from appwrite
...
// PUT route to update an image on appwrite (delete old and upload new)
